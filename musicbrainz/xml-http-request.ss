(module xml-http-request "swindle.ss"
  (require (only (lib "url.ss" "net")
                 call/input-url get-impure-port purify-port))
  (require (only (lib "head.ss" "net") extract-all-fields))
  (require (only (lib "port.ss") copy-port))
  (require (only (lib "13.ss" "srfi") string-drop))
  (require (only (lib "dom.ss" "dom") read-dom))
  (provide (all-defined))
