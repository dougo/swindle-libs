...
(module convert "swindle.ss"
  ...
  (defmethod* (read-dom . port)
    (xml->dom (xml:read-xml . port)))
  ...
  (defmethod* (string->dom (s <string>))
    (read-dom (open-input-string s)))
  (defmethod* (string->dom/element (s <string>) (parent <node>))
    (read-dom/element (open-input-string s) parent))
  (defmethod* (dom->string (node <node>))
    (with-output-to-string (thunk (write-dom node))))


  (defmethod* (xml->dom xml &opt parent)
    (let ((doc (and parent (or (owner-document parent) parent))))
      (cond ((xml:document? xml) (xml:document->dom xml))
	    ((xml:document-type? xml) (xml:document-type->dom xml))
	    ((xml:element? xml) (xml:element->dom xml parent))
	    ((xml:attribute? xml) (xml:attribute->dom xml parent))
	    ((xml:pcdata? xml) (xml:pcdata->dom xml doc))
	    ((xml:cdata? xml) (xml:cdata->dom xml doc))
	    ((xml:entity? xml) (xml:entity->dom xml doc))
	    ((xml:pi? xml) (xml:pi->dom xml doc))
	    ((xml:comment? xml) (xml:comment->dom xml doc))
	    (else (error 'xml->dom "unknown XML value: ~v" xml)))))

  (defmethod* (xml:document->dom xml)
    (let* ((doc (create-document (make-xml-dom-implementation) #f "dummy" #f))
	...
	(append-child! doc (xml->dom xml doc)))
      (when dtd (append-child! doc (xml->dom dtd)))
  ...
  (defmethod* (xml:document-type->dom xml)
